...
import { Calendar, User, BookOpen } from 'lucide-react'; // Lucide icons for decorative elements
import PageIntro from '../utility/pageIntro';

// Register GSAP plugins
gsap.registerPlugin(ScrollTrigger);

const BlogDetailHeroSection = ({ title }) => {
    const sectionRef = useRef(null);
    const titleRef = useRef(null);
    const metaRef = useRef(null); // Ref for author and date
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            ...
            gsap.fromTo([titleRef.current, metaRef.current],
                { opacity: 0, y: 30 },
                {
                    opacity: 1,
                    y: 0,
    ...
    return (
        ...
        >
            ...
            <div className="absolute inset-0 z-0">
                <Image
                    src={"/images/blog.png"}
                    alt={title || "Blog Post Background"}
                    layout="fill"
                    objectFit="cover"
                    quality={90}
                ...
                <div className="absolute inset-0 bg-gray-900/60 to-blue-900/60"></div> {/* Darker, muted overlay */}
            </div>
            ...
            <div className="relative z-10 text-white px-4 max-w-4xl mx-auto">
                <h1 ref={titleRef} className="text-3xl md:text-5xl font-extrabold leading-tight mb-4 text-shadow-sm truncate">
