...
import { Stethoscope, ShieldCheck, HeartPulse } from 'lucide-react'; // Lucide icons for decorative elements
import PageIntro from '../utility/pageIntro';

// Register GSAP plugins
gsap.registerPlugin(ScrollTrigger);

const ServiceDetailHeroSection = ({ title }) => {
    ...
    return (
        <section
            ref={sectionRef}
            className="relative w-full h-[400px] flex items-center justify-center text-center font-inter"
        >
            {/* Background Image with Subtle Overlay */}
            <div className="absolute inset-0 z-0">
                <Image
                    src={"/images/aboutUsHero.jpg"} // Use provided image or a fallback
                    alt={title || "Service Detail Background"}
                    layout="fill"
                    objectFit="cover"
                    quality={90}
                ...
                <div className="absolute inset-0 bg-blue-900/60 to-indigo-900/60"></div> {/* Darker, muted overlay */}
            </div>
            ...
            <div ref={el => iconRefs.current[1] = el} className="absolute bottom-[20%] right-[10%] z-0 opacity-10 transform rotate-45">
                <ShieldCheck className="w-24 h-24 text-blue-300" />
            </div>
            <div ref={el => iconRefs.current[2] = el} className="absolute top-[40%] right-[20%] z-0 opacity-10 transform rotate-90">
                <HeartPulse className="w-28 h-28 text-blue-300" />
            </div>

            {/* Content */}
            <div className="relative z-10 text-white px-4 max-w-4xl mx-auto">
                <h1 ref={titleRef} className="text-3xl md:text-5xl font-extrabold leading-tight mb-4 text-shadow-sm">
                    {title || "Service Title"}
                </h1>
